...
namespace MediaService
{
    public partial class MediaService : ServiceBase
    {
        ...
        public static List<PowerInfo> Powerinfos = new List<PowerInfo>(); //实时电量LIST
        ...
        public static string HttpkafkaLogServerUrl = "";
        ...
        public static MongoDatabase MongoDatabasePower = null;
        ...
        private void InitThread()
        {
            while (true)//初始化完成后，退出循环，否侧定时尝试初始化。
            {
                ...
                try
                {
                    HttpkafkaLogServerUrl = ConfigurationManager.AppSettings["HttpkafkaLogServerUrl"];
                    ...
                    string mongoDBPower = ConfigurationManager.AppSettings["mongoDBPower"].ToString();
                    ...
                    try
                    {
                        ...
                        mongServerSet.MaxConnectionPoolSize = 1000; 
                        ...
                        MongoDatabasePower = mongoServer.GetDatabase(mongoDBPower);
                    }
                    catch (Exception err)
                    {
        ...
        private void WriteLoLaToSQLThread()
        {
            ...
            List<PowerInfo> mongoPower=new List<PowerInfo>();
            while (true)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (mongoGPS.Count > 0)
                        {
                            try
                            {
                                MongoCollection mongoCollection = mongoDataBase.GetCollection("gps_" + DateTime.Now.ToString("yyyyMMdd") + (DateTime.Now.Hour / 2 * 2).ToString("00"));//选择集合
                                mongoCollection.InsertBatch(typeof(GPSinfo), mongoGPS);
                                #region 发送到远程服务器 add by jyx
                    ...
                    if (talkinfo.Count > 0)
                    {
                        ...
                        if (mongoTalk.Count > 0)
                        {
                            ...
                            {
                                WriteLog("WriteToMongo错误_对讲记录：" + err.Message, true);
                            }
                            finally
                            {
                                mongoTalk.Clear();
                    ...
                    if (meitiinfo.Count > 0)
                    {
                        ...
                        if (mongoMeiti.Count > 0)
                        {
                            ...
                            {
                                WriteLog("WriteToMongo错误_媒体播放记录：" + err.Message, true);
                            }
                            finally
                            {
                                mongoMeiti.Clear();
                    ...
                    if (newsinfo.Count > 0)
                    {
                        ...
                        if (mongoNews.Count > 0)
                        {
                            ...
                            {
                                WriteLog("WriteToMongo错误_新闻播放记录：" + err.Message, true);
                            }
                            finally
                            {
                                mongoNews.Clear();
                    ...
                    while (logininfo.Count > 0)
                    {
                        ...
                        {
                            WriteLog("WriteToMongo错误_golo登录记录：" + err.Message, true);
                        }
                        finally
                        {
                    ...
                    while (golo6Logininfo.Count > 0)
                    {
                        ...
                        {
                            WriteLog("WriteToMongo错误_golo6登录记录：" + err.Message, true);
                        }
                        finally
                        {
                    ...
                    if (dianzaninfo.Count > 0)
                    {
                        ...
                        if (mongodz.Count > 0)
                        {
                            ...
                            {
                                WriteLog("WriteToMongo错误_点赞记录：" + err.Message, true);
                            }
                            finally
                            {
                                mongodz.Clear();
                    ...
                    if (callTalkInfo.Count > 0)
                    {
                        ...
                        if (mongoCallTalk.Count > 0)
                        {
                            ...
                            {
                                WriteLog("WriteToMongo错误_频道呼叫：" + err.Message, true);
                            }
                            finally
                            {
                                mongoCallTalk.Clear();
                    ...
                    if (responseCallTalk.Count > 0)
                    {
                        ...
                        if (mongoResponseCallTalk.Count > 0)
                        {
                            ...
                            {
                                WriteLog("WriteToMongo错误_频道呼叫应答：" + err.Message, true);
                            }
                            finally
                            {
                    ...
                    #region 电量
                    if (Powerinfos.Count < 1)
                    {
                        Thread.Sleep(500);
                    }
                    else
                    {
                        int powerCount = Powerinfos.Count;
                        if (powerCount > 100) powerCount = 100;
                        for (int i = 0; i < powerCount; i++)
                        {
                            if (Powerinfos[i] != null)
                                mongoPower.Add(Powerinfos[i]);
                        }
                        Powerinfos.RemoveRange(0, powerCount);
                        if (mongoPower.Count > 0)
                        {
                            try
                            {
                                foreach (var powerInfo in mongoPower)
                                {
                                    MongoCollection mongoCollection = MongoDatabasePower.GetCollection(string.Format("power_{0}", powerInfo.Uid));
                                    mongoCollection.Insert(typeof(PowerInfo), powerInfo);
                                }
                                //MongoCollection mongoCollection = MongoDatabasePower.GetCollection("power_" + DateTime.Now.ToString("yyyyMMdd") + (DateTime.Now.Hour / 2 * 2).ToString("00"));//选择集合
                                //mongoCollection.InsertBatch(typeof(PowerInfo), mongoPower);
                            }
                            catch (Exception err)
                            {
                                WriteLog("WritePowerToMongo错误_电量：" + err.Message, true);
                            }
                            finally
                            {
                                mongoPower.Clear();
                ...
                {
                    WriteLog("WriteToSQLThread错误_异常：" + err.Message, true);
                    Thread.Sleep(2000);
                }
            }
        }
    ...
    public class PowerInfo
    {
        ...
        /// <summary>
        /// 状态 0  代表设备刚上线  1、代表设备在线中 2、设备离线中
        /// </summary>
        ...
        //public PowerInfo(int power, int time, int state, int uid)
        //{
        //    Power = power;
        //    Time = time;
        //    State = state;
